...
os.makedirs("NOBUS_DH_attack", exist_ok=True)
with open("NOBUS_DH_attack/dh_values.txt", "w") as f:
    f.write(f"{p}\n{q}\n{n}\n")
...
# Save the DER file
with open("NOBUS_DH_attack/backdoored_dhparams.der", "wb") as f:
    f.write(der_encoded)

print("✅ Successfully generated `backdoored_dhparams.der` with correct ASN.1 encoding!")

# Verify that p-1 and q-1 are strictly B-smooth
print(f"Factorization of p-1: {factorint(p - 1)}")
print(f"Factorization of q-1: {factorint(q - 1)}")
# Convert back to PEM
#os.system("openssl dhparam -inform DER -in backdoored_dhparams.der -out backdoored_dhparams.pem")

# Verify with OpenSSL
#os.system("openssl dhparam -in backdoored_dhparams.pem -text -noout")
