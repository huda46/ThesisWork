...
def generate_multiprime_rsa_params(bits=30):
    """
...
def wiener_attack(N, e):
    """
    Wiener's attack on RSA for both 2-prime and 3-prime RSA.
    Uses continued fraction expansion to recover small d.
    """
    ...
    for conv in convergents:
        ...
        phi_N = (e * d - 1) // k  # Compute candidate phi(N)

        # Since we have 3 primes: check if phi_N is consistent with N = pqr
        possible_N = []
        for i in range(2, int(N ** (1/3)) + 1):
            if N % i == 0:  # Find divisors of N
                p = i
                remaining = N // p
                for j in range(2, int(remaining ** (1/2)) + 1):
                    if remaining % j == 0:
                        q = j
                        r = remaining // q
                        if p * q * r == N:
                            possible_N.append((p, q, r))
  

        for (p, q, r) in possible_N:
            computed_phi = (p - 1) * (q - 1) * (r - 1)
            if computed_phi == phi_N:
